...
// ver CÃ¡lculo de estructuras VIII.35
 ...
 public double BucklingOmega(double Nxx, double[] G, double[] B) throws JMException{
  ...
  if(Nxx < 0.0 && G[AREA] > 0.0 && lBuckling)
  {
      ...
      double lo  = B[L_] * G[BLijY_] / iy;
      // esbeltez maxima entre para los ejes y, z
      lo = Math.min(lo, B[L_] * G[BLijZ_] / iz);
	      ...
	      else if(G[TypeMaterial_]== 2)  // F-24 steel 24 MN/m2
	      {
	    	  ...
	    	  // double l = -2.3443 + 1.07817 * lo - 0.0036181 * Math.pow(lo, 2) + 0.000008209862 * Math.pow(lo, 3) - 0.00000001058458 * Math.pow(lo, 4) + 0.000000000007147864 * Math.pow(lo, 5) - 0.00000000000000196410755 * Math.pow(lo, 6);
              ...
              if ( l <= 150)
              {
            	  w = 1.113 + 0.0070516 * l - 0.000132108 * Math.pow(l, 2.0) + 0.000002106132 * Math.pow(l, 3.0) - 0.00000000397368332151 * Math.pow(l, 4.0);
              }
              else w = 25.0;
	      }
          ...
          else if(G[TypeMaterial_] == 20)  // Wood hard an halt-hard
	      {
             ...
             if (l<=150)
             {
                 w = 1.048 + 0.005524 * l - 0.000101666 * Math.pow(l, 2.0) + 0.00000301687 * Math.pow(l, 3.0) - 0.000000004366246 * Math.pow(l, 4.0);
             }
             else w = 25.0;
	      }
      }
